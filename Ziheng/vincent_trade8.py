...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        for product in state.order_depths:
            ...
            traderData = self.append_last_x_avg(traderData, product, average, avg_hist_long)   # update traderData to include this timestep's average
            ...
            # calculate for 45 values
            
            if len(traderData["avg"][product]) >= avg_hist_long: # must check there is at least 15 data points
                x = [i for i in range(avg_hist_long)]
                y = traderData["avg"][product]
                gradient_long, c_long = np.polyfit(np.array(x), np.array(y), 1)   # finding lin reg equation
                ...
                for i in range(len(traderData["avg"][product])):
                    curr_avg = traderData["midprice_dict"][product][-i -1]
                    c_lin_reg = gradient_long * (avg_hist_long - i) + c_long
                    diff = c_lin_reg - curr_avg
                    diff_lst.append(diff)
                sd = np.std(diff_lst, 0)
            ...
            if len(traderData["avg"][product]) >= avg_hist:
                if mid_price < next_avg_price - sd_multiplier* sd and qty_to_mm != 0 and product not in ["STRAWBERRIES", "ROSES", "CHOCOLATE", "GIFT_BASKET", "ORCHIDS"] and norm:
                    ...
                    if curr_pos < 0:    # if we are short
                        ...
                        #     if product in ["CHOCOLATE", "STARBERREIS", "ROSES", "GIFT_BASKET"]:
                        #         orders.append(Order(product, market_buy_orders[0][0], -curr_pos))
